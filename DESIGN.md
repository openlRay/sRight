...
- Prefer HeroUI components for standard interface primitives, including `Table`, `Pagination`, `Toast`, modal-like surfaces, buttons, inputs, selects, and other reusable controls.
- Do not hand-roll common widgets when HeroUI already provides a stable component. Wrap or lightly style HeroUI components to match this design system instead.
- Use custom CSS only for application-specific layouts, GitLab-style diff presentation, and domain-specific interactions that HeroUI does not cover directly.
